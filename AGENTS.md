...
| **`derp-session.sh`** | **GDM `Exec=`** session: DRM compositor, optional **`--command`** chain (**`file://…/shell/dist/index.html`** → **`launch-cef-to-compositor.sh`** → **`cef_host`**). Resolves **`ROOT`** via canonical path (symlink-safe). Env defaults: **`DERP_ALLOW_SHELL_SPAWN=1`**, **`DERP_SHELL_WATCHDOG_SEC=5`**, **`RUST_LOG`** merges **`derp_input=debug`** and **`derp_shell_osr=info`**; **`DERP_PERF_SESSION=1`** adds **`shell_ipc=trace`** + **`CEF_HOST_PERF`**. **`DERP_SESSION_DMABUF_LOGS=1`** (opt-in via **`derp-session.local.env`**) adds **`derp_shell_dmabuf=debug`**, **`CEF_HOST_DMABUF_TRACE`**, **`CEF_HOST_CHROMIUM_VERBOSE`**. Log file **`DERP_COMPOSITOR_LOG`**: **truncate on start** unless **`DERP_COMPOSITOR_LOG_APPEND=1`**. Supervisor loop: **`DERP_COMPOSITOR_RESPAWN=1`** (default) respawns unless exit code **42** (SIGUSR2 reload). Override binaries: **`COMPOSITOR_BIN`**, **`CEF_HOST_BIN`**, socket **`DERP_WAYLAND_SOCKET`**, disable overlay **`DERP_SESSION_SHELL=0`**. |
