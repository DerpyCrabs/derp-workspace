    ...
    impl V8Handler {
        fn execute(
            ...
            match op.as_str() {
                ...
                "quit" => {}
                "backed_window_open" | "hosted_window_open" | "workspace_mutation" | "shell_hosted_window_state" | "shell_hosted_window_title" => {
                    let Some(a1) = args.get(1).and_then(|a| a.as_ref()) else {
                        return_exception!(
                            "hosted_window_open/backed_window_open/workspace_mutation/shell_hosted_window_state/shell_hosted_window_title requires JSON string"
                        );
                    };
                    if a1.is_string() == 0 {
                        return_exception!(
                            "hosted_window_open/backed_window_open/workspace_mutation/shell_hosted_window_state/shell_hosted_window_title: second arg must be a string"
                        );
                    }
