    ...
    impl V8Handler {
        fn execute(
            ...
            match op.as_str() {
                ...
                "quit" => {}
                "backed_window_open" | "hosted_window_open" | "workspace_mutation" | "shell_hosted_window_state" => {
                    let Some(a1) = args.get(1).and_then(|a| a.as_ref()) else {
                        return_exception!(
                            "hosted_window_open/backed_window_open/workspace_mutation/shell_hosted_window_state requires JSON string"
                        );
                    };
                    if a1.is_string() == 0 {
                        return_exception!(
                            "hosted_window_open/backed_window_open/workspace_mutation/shell_hosted_window_state: second arg must be a string"
                        );
                    }
