...
type ShellWindowGestureRuntimeOptions = {
  ...
  workspaceFindMonitorForTiledWindow: (
    windowId: number,
  ) => { outputName: string; outputId: string | null } | null;
  requestSharedStateSync: (
    request: ShellSharedStateSyncRequest,
    timing?: "now" | "microtask",
  ) => void;
...
export function createShellWindowGestureRuntime(
  options: ShellWindowGestureRuntimeOptions,
) {
  const [assistOverlay, setAssistOverlay] =
    createSignal<AssistOverlayState | null>(null);
  const [snapAssistPicker, setSnapAssistPicker] =
    createSignal<SnapAssistPickerState | null>(null);
  const [snapPickerHoverSelection, setSnapPickerHoverSelection] =
    createSignal<SnapPickerSelection | null>(null);
  ...
  function assistOverlayEqual(
    a: AssistOverlayState | null,
    b: AssistOverlayState | null,
  ) {
    if (a === b) return true;
    ...
    if (a.kind === "assist" && b.kind === "assist") {
      const hoverEqual =
        a.hoverSpan === null
          ? b.hoverSpan === null
          : b.hoverSpan !== null &&
            assistGridSpansEqual(a.hoverSpan, b.hoverSpan);
      return a.shape === b.shape && a.gutterPx === b.gutterPx && hoverEqual;
    }
    if (a.kind === "custom" && b.kind === "custom") {
      return (
        a.layout.id === b.layout.id && a.selectedZoneId === b.selectedZoneId
      );
    }
    return false;
  }
  ...
  function setAssistOverlayState(next: AssistOverlayState | null) {
    setAssistOverlay((current) =>
      assistOverlayEqual(current, next) ? current : next,
    );
  }
  ...
  function resolveSnapAssistContextFromScreens(
    ...
    const screen =
      (preferredMonitorName
        ? screens.find((entry) => entry.name === preferredMonitorName)
        : undefined) ??
    ...
    if (
      !screen ||
      !monitorLayout ||
      monitorLayout.layout.type !== "manual-snap"
    )
      return null;
    const reserveTaskbar = options.reserveTaskbarForMon(screen);
    const workGlobal = monitorTileFrameAreaGlobal(
      screen,
      reserveTaskbar,
      screen.taskbar_side,
    );
    ...
    return {
      ...
      screen,
      workGlobal,
      workCanvas: rectGlobalToCanvasLocal(
        workGlobal.x,
        workGlobal.y,
        workGlobal.w,
        workGlobal.h,
      ...
      customLayout:
        snapLayout.kind === "custom"
          ? (monitorLayout.customLayouts.find(
  ...
  function resolveSnapAssistContext(
    ...
    return resolveSnapAssistContextFromScreens(
      windowId,
      screens,
      origin,
      preferredMonitorName,
  ...
  function readDragMeasure(windowId: number): ShellDragMeasureCache | null {
    const drag =
      shellWindowDrag?.windowId === windowId ? shellWindowDrag : null;
    ...
    const pickerEl = main.querySelector(
    ...
    if (drag?.measure) {
      ...
      drag.measure.screens = screensListForLayout(
        options.screenDraftRows(),
        output,
        origin,
      );
  ...
  function dragContextForMonitor(
    ...
    if (measure.contexts.has(monitorName))
      return measure.contexts.get(monitorName) ?? null;
    const context = resolveSnapAssistContextFromScreens(
      windowId,
      measure.screens,
      measure.origin,
      monitorName,
  ...
  function applySnapPickerSelection(
    context: SnapAssistContext,
    selection: SnapPickerSelection | null,
  ) {
    if (!selection) {
      clearSnapAssistSelection();
      return;
    ...
    if (selection.hoverSpan && selection.shape) {
      ...
      const layout =
        context.customLayouts.find(
          (entry) => entry.id === snapLayout.layoutId,
        ) ?? null;
      if (parsed && layout) {
        setAssistOverlayState({
          kind: "custom",
  ...
  function updateSnapAssistFromSpan(
    ...
    const { zone, previewRect } = snapZoneAndPreviewFromAssistSpan(
      span,
      shape,
      context.workGlobal,
    );
  ...
  function updateSnapAssistFromCustomZone(
    ...
    const previewRect = resolveCustomLayoutZoneBounds(
  ...
  function commitSnapAssistSelection(windowId: number, closePicker = false) {
    ...
    const fallbackCandidate =
      ...
      Math.abs(drag.lastY - recentSnapCandidate.pointerY) <= 24
        ...
        : null;
    const snapWindowId =
      activeSnapWindowId ?? fallbackCandidate?.windowId ?? windowId;
    ...
    const droppedPreviewGlobal = activeSnapPreviewGlobal
      ? { ...activeSnapPreviewGlobal }
      : fallbackCandidate
        ? { ...fallbackCandidate.previewGlobal }
        : null;
    resetSnapAssistState();
    setAssistOverlayState(null);
    ...
    const occupied = options.occupiedSnapZonesOnMonitor(
      snapScreen,
      snapWindowId,
    );
    const globalBounds =
  ...
  function assistSpanFromPickerTarget(
    target: EventTarget | null,
    pickerEl: HTMLElement,
  ): AssistGridSpan | null {
    if (!(target instanceof HTMLElement)) return null;
    const spanEl = target.closest("[data-assist-grid-span]");
    if (!(spanEl instanceof HTMLElement) || !pickerEl.contains(spanEl))
      return null;
    ...
    if (![gridCols, gridRows, gc0, gc1, gr0, gr1].every(Number.isFinite))
      return null;
    return {
  ...
  function assistGridShapeFromValue(
    value: string | null,
  ): AssistGridShape | null {
    switch (value) {
      case "3x2":
      case "3x3":
      case "2x2":
  ...
  function assistSpanFromPickerPoint(
    ...
    if (!(gridEl instanceof HTMLElement) || !pickerEl.contains(gridEl))
      return null;
    const shape = assistGridShapeFromValue(
      gridEl.getAttribute("data-assist-master-grid"),
    );
    if (!shape) return null;
    const gutterPx = Number(
      gridEl.getAttribute("data-assist-master-grid-gutter-px"),
    );
    return assistSpanFromMasterGridPoint(
      gridEl,
      clientX,
      clientY,
      shape,
      Number.isFinite(gutterPx) ? gutterPx : 0,
    );
  }
  ...
  function customSelectionFromPickerTarget(
    ...
    if (!(zoneEl instanceof HTMLElement) || !pickerEl.contains(zoneEl))
      return null;
  ...
  function updateSnapAssistFromPickerPointer(
    ...
    for (const target of targets) {
      const pointSpan = assistSpanFromPickerPoint(
        target,
        pickerEl,
        clientX,
        clientY,
      ...
      const selection = customSelectionFromPickerTarget(
        target,
        context,
        pickerEl,
      );
  ...
  function beginShellWindowMove(
    ...
    options.shellMoveLog("titlebar_begin_request", {
      windowId,
      clientX,
      clientY,
    });
    ...
    if (!options.shellWireSend("move_begin", windowId)) {
      options.shellMoveLog("titlebar_begin_aborted", {
        windowId,
        reason: "no __derpShellWireSend",
      });
      return;
    }
    const stripEl = main?.querySelector(
    ...
    options.shellMoveLog("titlebar_begin_armed", {
      windowId,
      clientX,
      clientY,
    });
  ...
  function adoptShellWindowMove(
    ...
    options.shellMoveLog("titlebar_begin_adopted", {
      windowId,
      clientX,
      clientY,
      moved,
  ...
  function updateDragSnapAssist(
    ...
    const monitor = pickScreenForPointerSnap(
      global.x,
      global.y,
      measure.screens,
    );
    const context = monitor
      ? dragContextForMonitor(windowId, measure, monitor.name)
      : null;
    const pickerOpen =
      snapAssistPicker()?.source === "strip" &&
      snapAssistPicker()?.windowId === windowId;
    if (measure.pickerEl && measure.pickerRect) {
      ...
      if (
        ...
        if (pickerOpen && context) {
          updateSnapAssistFromPickerPointer(
            context,
            measure.pickerEl,
            clientX,
            clientY,
    ...
    if (superHeld) {
      if (context.assistShape) {
        const span = assistSpanFromWorkAreaPoint(
          pxForAssist,
          pyForAssist,
          context.assistShape,
          work,
    ...
    const zone = context.assistShape
      ? hitTestSnapZoneGlobal(global.x, global.y, work, context.assistShape)
      : null;
    updateSnapAssistFromEdgeZone(context, zone);
  }

  function updateShellWindowMovePointer(
    clientX: number,
    clientY: number,
    superHeld: boolean,
  ) {
    ...
    if (dClientX !== 0 || dClientY !== 0) {
      ...
      if (dx !== 0 || dy !== 0) {
        ...
        if (shellMoveDeltaLogSeq <= 12 || shellMoveDeltaLogSeq % 30 === 0) {
          options.shellMoveLog("titlebar_delta", {
            seq: shellMoveDeltaLogSeq,
            dx,
            dy,
            clientX,
            clientY,
  ...
  function beginShellWindowResize(
    windowId: number,
    edges: number,
    clientX: number,
    clientY: number,
    ...
    if (monitor !== null) {
      const rects = options.workspaceTiledRectMap(
        monitor.outputName,
        monitor.outputId,
      );
      ...
      for (const dir of dirs) {
        for (const neighborId of findEdgeNeighborsInMap(
          rects,
          windowId,
          dir,
          tolerance,
    ...
    if (useTiledPropagate && monitor !== null) {
      ...
      options.shellMoveLog("resize_begin_tiled", {
        windowId,
        edges,
        clientX,
        clientY,
  ...
  function updateShellWindowResizePointer(
    clientX: number,
    clientY: number,
    sendDelta: boolean,
  ) {
    ...
    if (shellWindowResize.kind === "compositor") {
      ...
      if (
        sendDelta &&
        (shellResizeDeltaLogSeq <= 12 || shellResizeDeltaLogSeq % 30 === 0)
      ) {
        options.shellMoveLog("resize_delta", {
          seq: shellResizeDeltaLogSeq,
          dx,
          dy,
          clientX,
          clientY,
  ...
  function applyShellWindowResize(
    clientX: number,
    clientY: number,
    _buttons?: number,
  ) {
  ...
  const snapStripState = createMemo<SnapAssistStripState | null>(() => {
    ...
    return {
      ...
      open:
        snapAssistPicker()?.source === "strip" &&
        snapAssistPicker()?.windowId === context.windowId,
    };
  });
