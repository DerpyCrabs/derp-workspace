...
function windowForPid(compositor: CompositorSnapshot, pid: number): WindowSnapshot | null {
  return compositor.windows.find((window) => window.wayland_client_pid === pid) ?? null
}

async function visiblePixelSummary(pathname: string): Promise<{ variedPixels: number; brightPixels: number }> {
  ...
  for (let index = 0; index < png.data.length; index += 4) {
    ...
    if (alpha > 0 && Math.max(r, g, b) - Math.min(r, g, b) > 16) variedPixels += 1
...
async function spawnWleirdProcess(
  ...
  const statusPath = path.join(artifactDir(), `wleird-${client}-${stamp}.status`)
  const outputPath = path.join(artifactDir(), `wleird-${client}-${stamp}.log`)
  const script = [
    `command -v ${shellQuote(`wleird-${client}`)} >/dev/null`,
    `stdbuf -oL -eL wleird-${client} ${args.map(shellQuote).join(' ')} >${shellQuote(outputPath)} 2>&1 &`,
    'pid=$!',
...
async function closeWleirdWindow(base: string, windowId: number, pid: number): Promise<void> {
...
export default defineGroup(import.meta.url, ({ test }) => {
  test('damage-paint submits visible fine-grained content', async ({ base, state }) => {
    const probe = await spawnWleirdWindow(base, state, 'damage-paint', ['fine-grid'])
    try {
      await syncTest(base)
      const settled = await getJson<CompositorSnapshot>(base, '/test/state/compositor')
      ...
      assert(pixels.variedPixels > 200 || pixels.brightPixels > 200, 'wleird-damage-paint did not render visible client content')
  ...
  test('disobey-resize stays mapped and decorated after compositor resize', async ({ base, state }) => {
    const probe = await spawnWleirdWindow(base, state, 'disobey-resize', ['0.5'])
    try {
      await derpctl([
        'window',
        'move',
      ...
      const resized = await waitFor(
        ...
        async () => {
          ...
          const controls = snapshots.shell.window_controls?.find((entry) => entry.window_id === probe.window.window_id)
          return window && controls?.titlebar && controls.close ? { compositor: snapshots.compositor, shell: snapshots.shell, window, controls } : null
        },
        5000,
        40,
      )
      assert(resized.window.width === 480 && resized.window.height === 360, 'wleird-disobey-resize compositor geometry did not settle')
  ...
  test('frame-callback receives repeated compositor frame callbacks', async ({ base, state }) => {
  ...
  test('surface output enters update when moved between monitors', async ({ base, state }) => {
    const probe = await spawnWleirdWindow(base, state, 'surface-outputs')
    try {
      ...
      const compositor = await getJson<CompositorSnapshot>(base, '/test/state/compositor')
      const outputs = [...compositor.outputs].sort((a, b) => a.x - b.x || a.y - b.y || a.name.localeCompare(b.name))
      const current = windowForPid(compositor, probe.pid)
      assert(current, 'wleird-surface-outputs disappeared before move')
      const target = outputs.find((output) => output.name !== current.output_name) ?? outputs[0]
      ...
      const moved = await waitFor(
        ...
        async () => {
          ...
          return window && window.output_name === target.name ? { compositor: next, window } : null
        },
        5000,
        40,
      )
  ...
  test('unmap removes the native window without wedging state', async ({ base, state }) => {
    ...
    try {
      const gone = await waitFor(
        ...
        async () => {
          const snapshots = await getSnapshots(base)
          const compositorWindow = windowForPid(snapshots.compositor, probe.pid)
          const shellWindow = snapshots.shell.windows.find((window) => window.window_id === probe.window.window_id)
  ...
  test('slow configure acknowledgements still get shell chrome', async ({ base, state }) => {
    ...
    try {
      const decorated = await waitFor(
        ...
        async () => {
          const snapshots = await getSnapshots(base)
          const controls = snapshots.shell.window_controls?.find((entry) => entry.window_id === probe.window.window_id)
          return controls?.titlebar && controls.close ? { shell: snapshots.shell, controls } : null
        },
        2500,
        40,
      )
  ...
  test('sigbus client does not crash the compositor', async ({ base }) => {
    const probe = await spawnWleirdProcess(base, 'sigbus')
    try {
      const status = await waitFor(
        'wait for wleird sigbus client exit',
        async () => {
          const text = await readText(probe.statusPath)
          return text?.match(/^exit:(\d+)$/m) ? text : null
        },
        5000,
        40,
      )
      const snapshots = await waitFor(
        'wait for wleird sigbus cleanup',
        async () => {
          const next = await getSnapshots(base)
          return windowForPid(next.compositor, probe.pid) ? null : next
        },
        5000,
        40,
      )
