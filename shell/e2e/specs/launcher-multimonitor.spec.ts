  ...
  test('programs menu opens searches and optionally launches a terminal app', async ({ base, state }) => {
    ...
    if (!launcherCandidate) {
      ...
      await waitFor(
        ...
        async () => {
          const shell = await getJson<ShellSnapshot>(base, '/test/state/shell')
          return shell.programs_menu_query === launcherCandidate.query ? shell : null
        },
        5000,
        100,
      )
      ...
      try {
        ...
        printNote(`launcher launch path skipped: ${error instanceof Error ? error.message : String(error)}`)
      }
    }
