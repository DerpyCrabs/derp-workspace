...
The tree includes a **minimal Smithay compositor** (winit/nested and headless, tests, CI), a **Phase‑3 slice** for shell pixels (`shell_wire` + Unix socket + compositor overlay), a **SolidJS** app in [`shell/`](shell/), and the **`cef_host`** binary (CEF windowless / OSR → that IPC). **Shell → compositor** control messages include **dma-buf frames**, move, **list windows**, **set geometry**, **close**, **fullscreen**, and **quit session**; the Solid shell exposes these via HTTP control → shell socket where applicable.
...
| `shell_wire/` | Length‑prefixed messages (`msg_type` + payload): BGRA frames, spawn, shell IPC move/geometry/close/fullscreen/quit/list, compositor→shell output geometry, window events, pointer |
...
1. **One-command install (repo anywhere, e.g. `~/derp-workspace`):** `bash scripts/install-system.sh` — `git pull` (unless `--no-git` / `INSTALL_SKIP_GIT=1`), then **`exec` of `scripts/install-system-run.sh`** so anything updated by the pull (including these scripts) drives the rest: `cargo build --release -p compositor -p cef_host`, `npm` for `shell/dist`, install into `/usr/local`, symlink `derp-session`. **`derp-session.sh` resolves that symlink** (`readlink -f`) so `shell/dist` and CEF RUNPATH are read from the **clone**, not from `/usr/local`.
...
**Gray shell / CEF:** DRM **`derp-session`** appends compositor and **`cef_host`** stdout/stderr to **`~/.local/state/derp/compositor.log`** (override with **`DERP_COMPOSITOR_LOG`**). Inspect from a TTY, SSH, or live mount. Run `cef_host` with **`CEF_HOST_DIAG=1`** for layout checks; **`CEF_HOST_TRACE_PAINT=1`** for first OSR paint size; **`CEF_HOST_LOG_FILE`** / **`CEF_HOST_LOG_SEVERITY=verbose`** for Chromium’s log. **`on_load_error`** is always printed into that session log. Solid frames are **dma-buf only** (no SHM / legacy socket pixels). For dma-buf import / EGL debugging, set **`DERP_SESSION_DMABUF_LOGS=1`** in **`scripts/derp-session.local.env`** (adds **`derp_shell_dmabuf=debug`**, per-frame **`CEF_HOST_DMABUF_TRACE`**, and **`CEF_HOST_CHROMIUM_VERBOSE`**). **`CEF_HOST_PERF=1`** logs timing on stderr; compositor **`RUST_LOG`** can include **`shell_ipc=trace`** for drain/apply tracing. After **`DERP_PERF_SESSION=1`**, **`derp-session`** also sets **`CEF_HOST_PERF=1`** for the CEF `--command` chain.
...
**Shell stall watchdog:** While `cef_host` is connected, the compositor stops if **no** shell→compositor messages arrive for **`DERP_SHELL_WATCHDOG_SEC`** (default **5** in `derp-session` via `install-system`). A **static** OSR UI sends no frames; the compositor then issues **`MSG_COMPOSITOR_PING`** and expects **`MSG_SHELL_PONG`** from `cef_host` so idle shells still count as alive. Set **`DERP_SHELL_WATCHDOG_SEC=0`** to disable the watchdog.
